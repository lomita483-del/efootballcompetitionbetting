-- Ongoing safeguard (not just one-time backfill): every time resolve_open_bets()
-- runs, first reopen any bet currently stuck at 'lost' that does NOT actually have
-- a leg whose match has ended, is not void, and genuinely lost. This can happen if
-- a bet was ever settled 'lost' by a stale/incorrect read before its other legs
-- were resolved. Without this, such a bet stays wrongly "lost" forever even after
-- its individual leg results self-heal back to pending.
...
DECLARE
  ...
  void_factor NUMERIC;
BEGIN
  -- Reopen any bet wrongly stuck at 'lost' with no genuine ended/non-void losing leg,
  -- and which still has at least one unresolved or not-yet-ended leg.
  UPDATE public.bets bt SET status = 'open', settled_at = NULL, cashout_amount = NULL
   WHERE bt.status = 'lost'
     ...
     AND EXISTS (
       SELECT 1 FROM public.bet_selections bs
       JOIN public.matches mt ON mt.id = bs.match_id
       WHERE bs.bet_id = bt.id AND (mt.status IS DISTINCT FROM 'ended' OR bs.result IS NULL)
     );
