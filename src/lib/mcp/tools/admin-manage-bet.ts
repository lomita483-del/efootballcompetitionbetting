...
export default defineTool({
  ...
  inputSchema: {
    ...
    action: z.enum(["suspend", "unsuspend", "refund", "void", "delete", "force_won", "force_lost", "void_selection", "unvoid_selection"]).describe("Administrative ticket action."),
    ...
    selection_id: z.string().optional().describe("Selection UUID required to void or unvoid one ticket leg."),
  },
  ...
  handler: async (input, ctx) => auditedToolCall("admin_manage_bet", input, ctx, async () => {
    ...
    else if (input.action === "delete") response = await db.rpc("admin_delete_bet", { _bet_id: input.bet_id, _refund: input.refund_stake ?? false, _reason: input.reason });
    else if (input.action === "force_won") response = await db.rpc("settle_pay_winning_bet", { _bet_id: input.bet_id });
    else if (input.action === "force_lost") response = await db.from("bets").update({ status: "lost", settled_at: new Date().toISOString() }).eq("id", input.bet_id);
    else {
      if (!input.selection_id) return { content: [{ type: "text", text: "selection_id is required for selection void actions." }], isError: true };
      response = await db.rpc("admin_toggle_selection_void", { _bet_id: input.bet_id, _selection_id: input.selection_id, _void: input.action === "void_selection" });
    }
