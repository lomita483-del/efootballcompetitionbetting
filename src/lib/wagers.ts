...
export async function rejectWager(id: string, reason?: string) {
  const { data, error } = await supabase.rpc("p2p_reject_wager", {
    _wager_id: id,
...
export async function adminSettleWager(input: {
  ...
  const { data, error } = await supabase.rpc("p2p_settle_wager", {
    _wager_id: input.wager_id,
    _winner_id: input.winner_id ?? undefined,
    _is_draw: input.is_draw ?? false,
    _final_home: input.final_home ?? undefined,
    _final_away: input.final_away ?? undefined,
...
export async function adminRefundWager(wager_id: string, reason?: string) {
  const { data, error } = await supabase.rpc("p2p_refund_wager", {
    _wager_id: wager_id,
    _reason: reason ?? undefined,
  } as any);
  if (error) throw error;
...
export async function adminTerminateWager(wager_id: string, reason: string, refund = true) {
  const { data, error } = await supabase.rpc("p2p_admin_terminate", {
    _wager_id: wager_id,
    _reason: reason,
    _refund: refund,
  });
