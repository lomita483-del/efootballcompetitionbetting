 ...
 * prompt.
 *
 * FIT: object-contain — the video is never cropped. It's shown at the largest
 * size that fits the screen while keeping its full frame intact. If its shape
 * doesn't exactly match the screen's shape, plain black bars fill the leftover
 * space on whichever axis doesn't match — nothing is ever cut off.
 *
 * On portrait screens (phone held normally), the video is rotated 90° so a
 * landscape video reads correctly. On landscape screens (desktop, or phone
 * turned sideways), no rotation is applied.
 */
export function VideoAd() {
  ...
  const videoClass = isPortrait
    ? "absolute top-1/2 left-1/2 object-contain"
