...
export function AdvertisementRow({ placement }: { placement: "home" | "matches" }) {
  const [items, setItems] = useState<any[]>([]);
  useEffect(() => {
    const load = () => (supabase as any).from("advertisements").select("*").eq("is_active", true).in("placement", [placement, "both"]).order("display_order", { ascending: true }).limit(8).then(({ data }: any) => setItems(data ?? []));
    void load();
