 ...
 * prompt.
 *
 * FIT: the video fills the screen completely (object-cover) on both mobile and
 * desktop, with zero black gaps. Since the video's proportions won't always
 * exactly match the screen's, a small amount of cropping on one edge is the
 * unavoidable trade-off for a full, gapless fill.
 *
 * On portrait screens (phone held normally), the video is rotated 90° so a
 * landscape video reads correctly filling the screen. On landscape screens
 * (desktop, or phone turned sideways), no rotation is applied.
 */
export function VideoAd() {
  ...
  const videoClass = isPortrait
    ? "absolute top-1/2 left-1/2 object-cover"
