...
/** Reads the admin-configured "Tasks Page Background" (app_settings.tasks_bg_*) and renders it full-bleed behind the page content. */
function usePageBackground() {
  const [bg, setBg] = useState<{ url: string | null; fit: string; pos: string }>({ url: null, fit: "cover", pos: "center" });
  useEffect(() => {
    const apply = (d: any) => setBg({ url: d?.tasks_bg_url ?? null, fit: d?.tasks_bg_fit ?? "cover", pos: d?.tasks_bg_position ?? "center" });
        ...
        {bg.url && (
          <div className="pointer-events-none absolute inset-0 -z-10 overflow-hidden">
            <img
              ...
              style={{ objectFit: (bg.fit as any) || "cover", objectPosition: bg.pos || "center" }}
            />
