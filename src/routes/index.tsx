      ...
      <section className="container mt-6">
        <div className="grid gap-4 min-[500px]:gap-5 min-[500px]:grid-cols-[minmax(0,1fr)_minmax(0,200px)] lg:grid-cols-[1fr_300px] xl:grid-cols-[1fr_300px] items-start">
          <div className="space-y-5 min-w-0">
            ...
            {!loading && featuredFallback.length > 0 && <Carousel opts={{ loop: featuredFallback.length > 1 }} plugins={featuredFallback.length > 1 ? [Autoplay({ delay: 5000, stopOnInteraction: false })] : []}><CarouselContent>{featuredFallback.map((m) => <CarouselItem key={m.id}><FeaturedGoldenMatches matches={[m]} bgImage={m.featured_image_url} bgFit={m.featured_image_fit} bgPos={m.featured_image_position} rankByTeamId={gangRankByTeamId} /></CarouselItem>)}</CarouselContent>{featuredFallback.length > 1 && <><CarouselPrevious /><CarouselNext /></>}</Carousel>}
            {!loading && live.length > 0 && <div><SectionHeader icon={Flame} title="Live Now" subtitle="Live odds. Markets close round-by-round." /><div className="space-y-3 mt-4">{live.map((m) => <MatchCardLive key={m.id} match={m} variant="row" />)}</div></div>}
            {!loading && upcoming.length > 0 && <div><SectionHeader icon={Crosshair} title="Upcoming Matches" subtitle="Lock your picks before the round starts." /><div className="ecb-match-list mt-4"><div className="ecb-odds-head"><span>Competition / Match</span><b>1</b><b>X</b><b>2</b></div>{upcoming.slice(0, 6).map((match) => <ArenaMatchRow key={match.id} match={match} />)}<div className="px-3 py-3 border-t border-border/60 text-right"><Link to="/matches" className="inline-flex items-center gap-1 text-[11px] font-bold uppercase tracking-widest text-muted-foreground hover:text-primary transition">View All <ChevronRight className="h-3.5 w-3.5" /></Link></div></div></div>}
          </div>
          <aside className="space-y-4 min-w-0 lg:sticky lg:top-20 self-start"><NewsSlider /><LotteryResultsCard /><MiniLiveLeaderboard /><GrandPrizeWinners /></aside>
        </div>
      </section>
      <PromoFooterBand />
    </Layout>
 ...
 * card that used to sit on the right has been removed.) If you upload a
 * hero background image via Branding settings (settings.hero_bg_url), it
 * still renders as a full-bleed background layer behind this whole section —
 * that's how you'd get the trophy/player artwork look from the reference. */
 ...
 *
 * Odds boxes are neutral dark cards with cream/white values — matching the
 * reference image exactly (green is reserved for the "Place Bet" action only,
 * gold/primary is used for the "selected" highlight, matching the same
 * selection color used everywhere else on the site, e.g. UpcomingMatchesTable).
 *
                  ...
                  return (
                    <button
                      ...
                      className={`flex flex-col items-center justify-center gap-1 rounded-xl py-3 px-2 min-h-[64px] backdrop-blur-md transition shadow-[inset_0_1px_0_rgba(255,255,255,0.06),0_4px_12px_-8px_rgba(0,0,0,0.7)] disabled:opacity-40 ${
                        selected
                          ? "bg-primary/20 border-2 border-primary ring-2 ring-primary/50 text-primary"
                          : "bg-black/45 border-2 border-white/15 hover:border-primary/50 hover:bg-black/60 text-amber-50"
                      }`}
                    >
                      <span className={`text-[10px] uppercase tracking-wider font-bold truncate max-w-full leading-none ${selected ? "text-primary/80" : "text-white/50"}`}>{o.label}</span>
                      <span className={`font-mono font-black text-lg drop-shadow-[0_1px_3px_rgba(0,0,0,0.9)] leading-none ${selected ? "text-primary" : "text-amber-50"}`}>{Number(o.value).toFixed(2)}</span>
                    </button>
                  );
                })}
              </div>
