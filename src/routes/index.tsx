      ...
      <section className="container mt-6">
        <div className="grid gap-4 min-[500px]:gap-5 min-[500px]:grid-cols-[minmax(0,1fr)_minmax(0,200px)] lg:grid-cols-[1fr_300px] xl:grid-cols-[1fr_300px] items-start">
          <div className="space-y-5 min-w-0">
            ...
            {!loading && upcoming.length > 0 && <div><SectionHeader icon={Crosshair} title="Upcoming Matches" subtitle="Lock your picks before the round starts." /><UpcomingMatchesTable matches={upcoming.slice(0, 6)} /></div>}
            {categoryGroups.map(([id, g]) => <div key={id}><SectionHeader icon={Crosshair} title={g.name} subtitle={`${g.items.length} match${g.items.length === 1 ? "" : "es"} in this category.`} /><div className="space-y-3 mt-4">{g.items.map((m) => <MatchCardLive key={m.id} match={m} variant="row" />)}</div></div>)}
          </div>
