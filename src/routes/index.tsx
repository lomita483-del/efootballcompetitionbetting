...
import {
  ...
  Users, Radio, Award, Shield, Handshake, Target, Sparkles, Twitter, Instagram, Youtube, MessageCircle,
} from "lucide-react";
...
function Index() {
  ...
  // Real per-team goal totals from ended matches, for the "Top Highlights" tiles.
  const topScoringTeams = useMemo(() => {
    const totals = new Map<string, { name: string; logo: string | null; goals: number }>();
    for (const m of matches) {
      if (m.status !== "ended") continue;
      if (m.home_team) {
        const cur = totals.get(m.home_team.id) ?? { name: m.home_team.name, logo: m.home_team.logo_url, goals: 0 };
        cur.goals += m.home_score ?? 0;
        totals.set(m.home_team.id, cur);
      }
      if (m.away_team) {
        const cur = totals.get(m.away_team.id) ?? { name: m.away_team.name, logo: m.away_team.logo_url, goals: 0 };
        cur.goals += m.away_score ?? 0;
        totals.set(m.away_team.id, cur);
      }
    }
    return Array.from(totals.values()).sort((a, b) => b.goals - a.goals).slice(0, 2);
  }, [matches]);
  ...
  return (
    <Layout>
      ...
      {/* Top Highlights (real goal totals from ended matches) + Top Players, side by side like the reference layout */}
      <section className="container mt-6 grid lg:grid-cols-[1fr_1fr] gap-4 items-start">
        <TopHighlightsTiles teams={topScoringTeams} />
        <TrendingPlayers />
      </section>
      ...
      <section className="container mt-3">
        <div className="grid gap-3 min-[500px]:gap-5 min-[500px]:grid-cols-[minmax(0,1fr)_minmax(0,200px)] lg:grid-cols-[1fr_300px] xl:grid-cols-[1fr_300px] items-start">
          ...
          <aside className="space-y-6 min-w-0 lg:sticky lg:top-20 self-start">
            <NewsSlider />
            <div><div className="mt-3"><HotBets /></div></div>
...
/** Two big stat tiles for the top scoring teams — real totals computed from ended matches. */
function TopHighlightsTiles({ teams }: { teams: { name: string; logo: string | null; goals: number }[] }) {
  ...
  return (
    <Card className="glass-strong border-primary/25 overflow-hidden">
      ...
      <div className="grid grid-cols-2 gap-2 p-3">
        {teams.map((t) => (
          <div key={t.name} className="relative overflow-hidden rounded-xl border border-primary/25 bg-gradient-to-br from-primary/15 to-transparent p-4 text-center">
            <div className="mx-auto h-12 w-12 rounded-full border border-primary/40 bg-background/40 grid place-items-center overflow-hidden">
              {t.logo ? <img src={t.logo} alt={t.name} className="h-full w-full object-cover" /> : <span className="text-xs font-black text-primary">{t.name.slice(0, 2).toUpperCase()}</span>}
            </div>
            <div className="mt-2 text-2xl font-black gradient-gold-text leading-none">{t.goals}</div>
            <div className="text-[10px] uppercase tracking-widest text-muted-foreground">Goals in total</div>
            <div className="mt-1 text-[11px] font-bold truncate">{t.name}</div>
          </div>
        ))}
      </div>
    </Card>
