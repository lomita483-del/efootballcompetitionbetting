...
import { VideoAdsAdminPanel } from "@/components/admin/VideoAdsAdminPanel";
...
export function AdminPage() {
  const { isAdmin, isMod, profile, loading } = useAuth();
  ...
  useEffect(() => {
    const loadHero = () => {
      supabase.from("app_settings").select("admin_hero_url,admin_hero_fit,admin_hero_position").eq("id", 1).maybeSingle()
        .then(({ data, error }) => {
          if (error) { console.error("admin_hero fetch failed:", error.message); return; }
    ...
    loadHero();
    // Live-refresh the banner the instant Settings saves a new value —
    // don't rely on the admin having to reload the page.
    const ch = supabase
      .channel("admin-hero-bg")
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "app_settings", filter: "id=eq.1" }, loadHero)
      .subscribe();
  ...
  return (
    <Layout>
      <main className={`admin-console-page${unblurred ? " admin-unblurred" : ""} w-full overflow-x-hidden`}>
        <div className="admin-shell-aurora" aria-hidden="true" />
        ...
        <div className="w-full max-w-[1280px] ml-0 mr-auto px-3 sm:px-4 py-4 sm:py-6 space-y-4">
          ...
          >
            ...
            <div className="relative min-h-[120px] sm:min-h-[180px] flex flex-col justify-center">
              ...
              <h2 className="text-2xl sm:text-5xl font-black uppercase leading-[1.05] text-emerald-400 truncate">{profile?.full_name || "Admin"}</h2>
...
function AdsPanel() {
  ...
  async function add() {
    ...
    const placement = draft.placement === "all" ? "all" : `/${draft.placement.replace(/^\/+|\/+$/g, "")}`;
    const { error } = await supabase.from("advertisements").insert({ ...draft, placement, link_url: draft.link_url || null } as any);
    if (error) { toast.error(error.message); return; }
...
function AnalyticsPanel() {
  ...
  const [arenaImg, setArenaImg] = useState<string | null>(null);
  const [arenaFit, setArenaFit] = useState<string>("cover");
  const [arenaPos, setArenaPos] = useState<string>("center");

  useEffect(() => {
    const loadArena = () => {
      supabase.from("app_settings").select("league_arena_url,league_arena_fit,league_arena_position").eq("id", 1).maybeSingle()
        .then(({ data, error }) => {
          if (error) { console.error("league_arena fetch failed:", error.message); return; }
          setArenaImg((data as any)?.league_arena_url ?? null);
          setArenaFit((data as any)?.league_arena_fit ?? "cover");
          setArenaPos((data as any)?.league_arena_position ?? "center");
        });
    };
    loadArena();
    const arenaCh = supabase
      .channel("admin-league-arena")
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "app_settings", filter: "id=eq.1" }, loadArena)
      .subscribe();
    return () => { supabase.removeChannel(arenaCh); };
  }, []);
  ...
  // Tabs moderators are allowed to open (must match modOk items in AdminSidebar.tsx).
  const MOD_SAFE_TABS = new Set(["analytics", "users", "appeals", "chat", "tickets", "matches", "topbets", "withdrawals", "content", "spotlights", "notify"]);
  const goTab = (t: string) => {
    if (!isAdmin && !MOD_SAFE_TABS.has(t)) return;
  ...
  return (
    <div className="space-y-3">
      ...
      {/* ROW 7 — Broadcast Center | Live Game Stats | Top Platform + League Arena
          Two-column, height-matched layout: left column carries Broadcast +
          Live Game Stats side-by-side, then the 6 module tiles beneath them;
          right column stacks Top Platform on top of the League Arena banner
          so both columns bottom out at the same line. On mobile this reflows
          to a single column: Broadcast, Live Stats, Top Platform, League
          Arena, then the tiles as a readable 3-across grid. */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 items-start">
        ...
        <div className="lg:col-span-2 flex flex-col gap-3 order-1">
          ...
          {/* Module tiles — always 6-across in a single row (desktop-forced layout means
              this never needs a mobile-specific 3-across fallback anymore). */}
          <div className="grid grid-cols-6 gap-2 sm:gap-3">
            {[
              ...
              <Card key={m.l} className="border-primary/25 bg-card/60 p-1.5 sm:p-2 flex flex-col">
                <button type="button" onClick={() => goTab(m.t)} className="relative aspect-[16/9] w-full mb-1 rounded-lg overflow-hidden border border-primary/25 hover:border-primary/60 transition active:scale-95">
        ...
        {/* RIGHT COLUMN — Top Platform stacked above League Arena, matching the left column's total height */}
          ...
          <Card className="border-primary/25 bg-card/70 p-2.5 flex flex-col">
            <div className="mb-1.5 flex items-center justify-between gap-2">
              <div>
                ...
                <div className="text-[6px] sm:text-[8px] uppercase tracking-[0.25em] text-muted-foreground">Stats Overview</div>
              </div>
              <button onClick={() => nav({ to: "/leaderboard" })} className="text-[8px] sm:text-[10px] text-muted-foreground hover:text-primary shrink-0">View More Platforms</button>
            </div>
            <div className="flex items-center gap-2 rounded-lg border border-primary/20 bg-background/40 px-2 py-2">
              <Trophy className="h-4 w-4 shrink-0 text-primary" />
              <div className="min-w-0 flex-1">
                <div className="truncate text-[10px] sm:text-xs font-black uppercase tracking-wide text-foreground">E-Football Betting</div>
                <div className="truncate text-[7px] sm:text-[9px] text-muted-foreground">
                  Total Users {stats.totalUsers} · Total Volume {short(stats.circulating)} · Ranked #1
                </div>
              </div>
            </div>
          </Card>

          {/* League Arena — fills the rest of the right column so it bottoms out level with the left column */}
          <Card className="group relative min-h-[180px] flex-1 overflow-hidden border-primary/30 bg-card/60">
            <img
      ...
      {/* Quick actions moved to the bottom — filtered to modOk tabs for moderators */}
      <QuickActionsBar onOpen={goTab} actions={isAdmin ? QUICK_ACTIONS : QUICK_ACTIONS.filter((a) => MOD_SAFE_TABS.has(a.t))} />
    </div>
  );
}
...
const QUICK_ACTIONS: { i: any; l: string; t: string }[] = [
  ...
  { i: ImageIcon, l: "Ads Management", t: "ads" },
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-4 max-w-5xl">
      ...
      <div className="lg:col-span-2">
        <SettingsSection icon={ImageIcon} title="Ads moved" subtitle="Pop-out and full-screen video ads now live in their own Ads Management panel.">
          <div className="flex items-center justify-between gap-3 rounded-md border border-primary/30 bg-primary/10 p-3 text-xs">
            <span>Manage pop-out and video ads from the Ads Management shortcut on the Analytics quick-actions bar.</span>
            <Button size="sm" onClick={() => window.dispatchEvent(new CustomEvent("admin:set-tab", { detail: "ads" }))}>Open Ads Management</Button>
          </div>
          {s.popup_ad_active && (
            <div className="mt-2 flex items-center justify-between gap-3 rounded-md border border-amber-400/30 bg-amber-500/10 p-2 text-[11px]">
              <span>A legacy single pop-up ad is still enabled from the old system. It only shows while no ad in Ads Management is active.</span>
      ...
      <SettingsSection icon={ImageIcon} title="Backgrounds & Appearance" subtitle="Site-wide background and admin console header image — upload or paste a URL, then crop/fit and preview live.">
        ...
        />
        <ImageSettingControl
          label="League Arena image"
          value={s.league_arena_url}
          onChange={(url) => setS({ ...s, league_arena_url: url })}
          fit={s.league_arena_fit ?? "cover"}
          onFitChange={(v) => setS({ ...s, league_arena_fit: v })}
          position={s.league_arena_position ?? "center"}
          onPositionChange={(v) => setS({ ...s, league_arena_position: v })}
          aspect="16 / 10"
          help="Shown in the “League Arena” card on the admin dashboard (bottom right, skull-in-flames box by default). Animated GIF supported."
        />
        <ImageSettingControl
