import { Trophy, Medal as MedalIcon, ArrowUp, ArrowDown, Star, Users, Target, Goal, Download, Link2, Link2Off, CheckCircle2, XCircle, Users as UsersIcon, Crown, CalendarDays, Equal, CircleDot, UserCheck, UserX, Footprints, Award, X as XIcon } from "lucide-react";
...
function RankBadge({ rank, label }: { rank: number; label: string }) {
  ...
  return (
    <div className="flex-1 min-w-[92px] rounded-xl border-2 border-amber-400/70 bg-gradient-to-b from-amber-500/15 via-black/40 to-black/40 px-2 py-2.5 text-center shadow-[0_0_16px_-4px_rgba(212,175,55,0.5)]">
      <Award className="mx-auto h-4 w-4 text-amber-300 mb-0.5" />
      <div className="text-xl font-black text-amber-200 tabular-nums leading-tight">#{rank + 1}</div>
      <div className="text-[9px] uppercase tracking-widest text-amber-300/80 font-bold">{label}</div>
      <div className="flex justify-center gap-0.5 mt-1">
        {[0, 1, 2].map((s) => <Star key={s} className="h-2 w-2 fill-amber-400 text-amber-400" />)}
      </div>
    </div>
  );
}
  ...
  return (
    <div className="rounded-xl border-2 border-amber-400/70 bg-gradient-to-r from-amber-500/15 via-black/40 to-black/40 px-4 py-3 shadow-[0_0_20px_-6px_rgba(212,175,55,0.5)]">
      <div className="flex items-center gap-3">
        <div className="grid place-items-center h-11 w-11 rounded-full border-2 border-amber-400/70 bg-black/40 shrink-0">
          <Footprints className="h-5 w-5 text-amber-300" />
        </div>
      ...
      <DialogContent hideDefaultClose className="max-w-lg max-h-[85vh] overflow-y-auto rounded-2xl border-2 border-amber-400/60 bg-gradient-to-b from-[#0d0a05] via-[#0a0806] to-black p-5 shadow-[0_0_60px_-15px_rgba(212,175,55,0.5)]">
        ...
        <div className="flex flex-col items-center text-center pt-1 pb-3">
          <div className="relative">
            {isTopOfSomething && (
              <Crown className="absolute -top-4 left-1/2 -translate-x-1/2 h-6 w-6 text-amber-300 fill-amber-300 drop-shadow-[0_0_10px_rgba(212,175,55,0.8)]" />
            )}
            <div className="h-24 w-24 rounded-full p-1 bg-gradient-to-b from-amber-300 via-amber-500 to-amber-700 shadow-[0_0_30px_-4px_rgba(212,175,55,0.7)]">
        ...
        {!loading && profile && (
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              <RankBadge rank={teamRank} label="Team Rank" />
              <RankBadge rank={shooterRank} label="Shooter Rank" />
              <RankBadge rank={scorerRank} label="Scorer Rank" />
            </div>
