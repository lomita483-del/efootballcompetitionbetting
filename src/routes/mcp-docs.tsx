...
function McpDocsPage() {
  ...
  return <Layout><main className="mx-auto w-full max-w-6xl px-6 py-10">
    ...
    <section className={`grid gap-6 py-8 ${isAdmin ? "lg:grid-cols-2" : ""}`}><Card className="border-primary/20 bg-card/70 p-6"><div className="mb-5 flex items-center gap-3"><Sparkles className="h-5 w-5 text-primary" /><h2 className="text-2xl font-bold">Player tools</h2></div><div className="space-y-4">{playerTools.map(([name, description]) => <div key={name} className="border-b border-border/60 pb-4 last:border-0"><code className="text-sm font-bold text-primary">{name}</code><p className="mt-1 text-sm text-muted-foreground">{description}</p></div>)}</div></Card>{isAdmin && <Card className="border-accent/25 bg-card/70 p-6"><div className="mb-5 flex items-center gap-3"><LockKeyhole className="h-5 w-5 text-accent" /><h2 className="text-2xl font-bold">Super Admin tools</h2></div><div className="space-y-4">{adminTools.map(([name, description]) => <div key={name} className="border-b border-border/60 pb-4 last:border-0"><code className="text-sm font-bold text-accent">{name}</code><p className="mt-1 text-sm text-muted-foreground">{description}</p></div>)}</div></Card>}</section>
    <section className={`grid gap-6 pb-8 ${isAdmin ? "lg:grid-cols-2" : ""}`}><div><h2 className="mb-4 text-xl font-bold">Example player prompts</h2><div className="space-y-3">{userPrompts.map((prompt) => <Prompt key={prompt}>{prompt}</Prompt>)}</div></div>{isAdmin && <div><h2 className="mb-4 text-xl font-bold">Example Super Admin prompts</h2><div className="space-y-3">{adminPrompts.map((prompt) => <Prompt key={prompt}>{prompt}</Prompt>)}</div></div>}</section>
