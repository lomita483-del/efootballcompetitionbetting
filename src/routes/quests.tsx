...
export const Route = createFileRoute("/quests")({
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Quest — ECB" },
      ...
      { property: "og:url", content: "/quests" },
    ],
...
function QuestPage() {
  ...
  useEffect(() => {
    ...
    loadProgress();
    const ch = supabase
      .channel(`quest-progress-${user.id}`)
  ...
  if (!user) {
    return <Layout><div className="container py-10"><Link to="/login" className="text-primary underline">Sign in</Link> to view your quests.</div></Layout>;
  }
      ...
      <div className="container mx-auto max-w-3xl px-4 py-8">
        ...
        <div className="mb-6">
          ...
          <h1 className="mt-1 flex items-center gap-2 font-display text-3xl font-extrabold gradient-gold-text md:text-4xl">
            <ListChecks className="h-7 w-7 text-primary" />Quest
          </h1>
        ...
        {!loading && tasks.length === 0 && (
          <Card className="glass p-6 text-center text-muted-foreground"><Sparkles className="mx-auto mb-2 h-6 w-6 text-primary" />No active quests — check back soon.</Card>
        )}
